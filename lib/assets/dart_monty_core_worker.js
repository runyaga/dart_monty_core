...
async function initWasm() {
  ...
  const wireFormatVersion = typeof wasm2.monty_wire_format_version === "function" ? wasm2.monty_wire_format_version() : null;
  self.postMessage({
    type: "ready",
    wireFormatVersion,
...
function logMem(tag) {
  try {
    const mem = wasm2 && wasm2.memory ? wasm2.memory.buffer.byteLength : null;
    // Best-effort; keep it on the console so Chrome stderr captures it.
    console.log(`[DartMontyWorker] mem ${tag} memBytes=${mem}`);
    self.postMessage({ type: "debug", tag, memBytes: mem });
  } catch {
  }
}
function handleRun(id, code, limits, scriptName) {
  logMem(`before_run:${scriptName || ""}`);
  let cCode = null;
  let cName = null;
  let outError = null;
  let handle;
  ...
  outError.free();
  logMem(`after_create:${scriptName || ""}`);
  if (limits) {
...
function handleStart(id, code, extFns, limits, scriptName) {
  logMem(`before_start:${scriptName || ""}`);
  if (activeHandle) {
...
function handleStartPrecompiled(id, dataBase64, limits, scriptName) {
  // debug: memory snapshots per request
  logMem(`before_start_precompiled:${scriptName || ""}`);
  if (activeHandle) {
    wasm2.monty_free(activeHandle);
    activeHandle = null;
  }
...
function handleReplResumeWithException(id, replId, excType, errorMessage) {
  ...
  try {
    ...
    cErr = allocCString(errorMessage);
    outError = allocOutPtr();
    const tag = wasm2.monty_repl_resume_with_exception(handle, cExcType.ptr, cErr.ptr, outError.ptr);
    const errPtr = outError.read();
    const errMsg = readAndFreeCString(errPtr);
    self.postMessage(readProgress(id, handle, tag, errMsg, REPL_PROGRESS));
  } finally {
