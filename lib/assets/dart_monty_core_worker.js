...
function handleSnapshot(id) {
  ...
  let ptr;
  try {
    ptr = wasm2.monty_snapshot(activeHandle, outLen.ptr, outError.ptr);
  } catch (e) {
    outLen.free();
    outError.free();
    self.postMessage({
...
function handleCompile(id, code, scriptName) {
  ...
  let ptr;
  try {
    ptr = wasm2.monty_snapshot(handle, outLen.ptr, outError.ptr);
  } catch (e) {
    outLen.free();
    outError.free();
    wasm2.monty_free(handle);
...
function handleReplSnapshot(id, replId) {
  ...
  let ptr;
  try {
    ptr = wasm2.monty_repl_snapshot(handle, outLen.ptr, outError.ptr);
  } catch (e) {
    outLen.free();
    outError.free();
    self.postMessage({
...
function handleReplRestore(id, replId, dataBase64) {
  ...
  let handle;
  try {
    handle = wasm2.monty_repl_restore(ptr, bytes.length, 0, 0, outError.ptr);
  } catch (e) {
    outError.free();
    throw e;
  } finally {
