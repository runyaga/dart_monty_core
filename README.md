...
> **`dart_monty_core` builds the native FFI binary from source on
> `dart pub get`.** Every FFI consumer needs a working Rust toolchain
> (cargo + rustc), including Flutter consumers coming in via
> [`dart_monty`](https://github.com/runyaga/dart_monty). Mobile
> (iOS/Android) is not handled by `hook/build.dart` — see "Mobile
> (iOS, Android) is not handled by this package's hook" below for
